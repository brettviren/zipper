// This file provides various helper functions that assist in constructing "ported graphs".
//
// It originates from the Wire-Cell Toolkit:
//
//   https://github.com/WireCell/wire-cell-toolkit/
//
// With light modification to reduce dependencies.
// 
// The nomenclature:
//
//
// - node :: generally a vertex of the ported graph
//
// - port :: an input or output of a node accepting data of some type
// and semantic meaning.
//
// - inode :: a identified configuration object for an instance of
// some application type (eg C++ class).  The type may (and often
// does) implement an ported graph node type in the application or it
// may represent a non-graph-node (eg a "service").  An inode has
// three attributes: "type" is a string that an application maps to a
// native type, "name" which the application uses to identify an
// instance of that type and "data" which holds a configuration object
// following a schema expected by the type.
//
// - pnode :: a wrapper a set of input and output ports and a single
// inode or a number of other pnodes which expose a subset of their
// ports.  A pnode is used as an intermediate representation by
// pgraph.jsonnet and this wrapper is stripped on output.  A pnode has
// attributes "type" always of value "Pnode", "name" uniquely
// identifying the pnode in the graph, "edges" holding any internal
// edges if the pnode represents a composit subgraph and "uses"
// holding references to any inode.  References are of the form
// "type:name" of the inode.
//
// - graph :: the final graph is expressed as an array of inodes and
// an array of edges between any inodes that are also ported graph
// nodes.


{
    // An inode instance may be referenced as a string "type:name" or
    // just "type" if the instance has no name (empty string)
    ...
    port(inode, num=0) :: { node: $.tn(inode), port: num },
    ...
    edges(graph) :: $.unique_list(graph.edges),
    ...
    // lists.
    uses(graph) :: $.unique_list(self.resolve_uses(graph.uses)),
    

    // Return graph representation of {nodes:[...], edges:[...]} from
    // a closed, composite pnode.
    graph(pnode) :: {
        nodes: $.uses(pnode),
        edges: $.edges(pnode),
    },
